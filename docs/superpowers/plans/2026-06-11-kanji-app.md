...
Run: `node -e "const d=require('./data/raw/vocab-n5.json'); console.log(JSON.stringify(d[0],null,1)); console.log(d.length)"`
Expected: `word`(표기), `furigana`(읽기), `meaning`(영어 뜻) 류의 필드 확인. 실제 필드명이 다르면 아래 `RawWord`와 매핑 코드를 실물에 맞춘다.
...
const FIXTURE = JSON.stringify([
  { word: '休日', furigana: 'きゅうじつ', meaning: 'holiday' },
  { word: '休日', furigana: 'きゅうじつ', meaning: 'holiday (dup)' },
  { word: 'ねこ', furigana: '', meaning: 'cat' },          // 한자 없음 → 제외
  { word: '挨拶', furigana: 'あいさつ', meaning: 'greeting' }, // 범위 밖 한자 → 제외
  { word: '木', furigana: 'き', meaning: 'tree' },
]);
...
test('범위 내 한자로만 이루어진 단어를 추출하고 중복을 제거한다', () => {
  ...
  expect(out.find((w) => w.surface === '木')!.meaningKo).toBe('');
});
```
...
export function parseVocab(
  ...
  const raw = JSON.parse(json) as RawWord[];
  ...
  for (const w of raw) {
    ...
    out.set(surface, {
      surface,
      reading: w.furigana?.trim() || surface,
...
for (const level of LEVELS) {
  const words = parseVocab(
    raw(`vocab-${level.toLowerCase()}.json`), level, kanjiSet, wordsKoOv,
  );
